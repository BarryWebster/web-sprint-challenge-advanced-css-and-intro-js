...
function get20s(array) {
  ...
  const newYears = array[i].years;
  const yearBorn = newYears.split(' ');
  const yearDied = newYears.split(' ');
    if(yearBorn[0] >= 1900 && yearDied[2] <= 2000) {
      filteredArtistsArray.push(array[i].name);
    }
  }
